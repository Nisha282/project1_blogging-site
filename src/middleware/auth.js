...
exports.authentication = function (req, res, next) {
  try {
    ...
    if (!Token)
      return res.status(400).send({ status: false, message: "login is requred" });

    let tokenVerify = jwt.verify(Token, "FunctionUP-Project1-Group30");
    return next()
  } catch (err) {
    return res.status(500).send({ status: false, message: "Server Error 500" });
  }
};
...
exports.authorization = async function (req, res, next) {
  try {
    ...
    if (!checkBlogId) {
      return res.status(403).send({ status: false, message: "blogid is wrong" });
    }
